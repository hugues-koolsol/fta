    ...
    if((tab1.length == tab2.length)){
        ...
        for(i=0;(i < tab1.length);i=i+1){
            if((tab1[i] != tab2[i])){
                ...
                document.getElementById('global_messages').innerHTML+='<div class="yywarning">'+tab1[i].replaceAll(' ','░')+'</div>';
                document.getElementById('global_messages').innerHTML+='<div class="yywarning">'+tab2[i].replaceAll(' ','░')+'</div>';
        ...
        for(i=0;(goOn) && (i < tab1.length) && (i < tab2.length);i=i+1){
            if((tab1[i] != tab2[i])){
                global_messages[lines].push(i);
                document.getElementById('global_messages').innerHTML+='<div class="yywarning">ligne : '+(i+1)+'<br />'+tab1[i].replaceAll(' ','░')+'<br />'+tab2[i].replaceAll(' ','░')+'</div>';
                goOn=false;
            }
        }
...
function memeHauteur(normalise,source){
    ...
    document.getElementById(normalise).style.height=.height;
}
/*
  =====================================================================================================================
*/
...
function getCaretCoordinates(element,position){
    ...
    var properties= Array('direction','boxSizing','width','height','overflowX','overflowY','borderTopWidth','borderRightWidth','borderBottomWidth','borderLeftWidth','borderStyle','paddingTop','paddingRight','paddingBottom','paddingLeft','fontStyle','fontVariant','fontWeight','fontStretch','fontSize','fontSizeAdjust','lineHeight','fontFamily','textAlign','textTransform','textIndent','textDecoration','letterSpacing','wordSpacing','tabSize','MozTabSize');
...
function mettreEnCommentaire(){
    ...
    txtDeb+nouveauCommentaire+txtFin;
...
function initialisationEditeur(){
    ...
    for(i=0;(i < tabtext.length);i=i+1){
        global_editeur_debut_texte_tab.push(Array(j));
        ...
        if((global_editeur_debut_texte_tab.length >= 2)){
            ...
            if((textPrec != '')){
                ...
                if((pos >= 0)){
                    ...
                    this=global_editeur_debut_texte+toAdd+global_editeur_fin_texte;
                    global_editeur_derniere_valeur_selecStart=global_editeur_derniere_valeur_selecStart+toAdd.length-offSetBack;
                    this=global_editeur_derniere_valeur_selecStart;
                    this=global_editeur_derniere_valeur_selecStart;
                    global_editeur_derniere_valeur_selecStart=this;
                    global_editeur_derniere_valeur_selectEnd=this;
                    initialisationEditeur();
                    this=global_editeur_scrolltop;
                }
            }
        }else{
        }
