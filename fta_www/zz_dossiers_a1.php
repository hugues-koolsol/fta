...
if((isset($_GET['__action'])) && ($_GET['__action'] == '__suppression')){
    ...
    */
    ...
    $o1.='   <br /><br /><b>'.'('.$__valeurs['T0.chi_id_dossier'].')  nom : '.$__valeurs['T0.chp_nom_dossier'].'    <br /> '.'</b><br />'.PHP_EOL;
        ...
        if(is_dir($chemin_relatif)){
            ...
            if($monscan !== false){
                ...
                foreach($monscan as $k0 => $v0){
                    ...
                    if(($v0 !== '.') && ($v0 !== '..')){
                        ...
                        $glob[]=array( 'nom' => $chemin_du_repertoire.$v0, 'type_fichier' => @is_file($chemin_du_repertoire.$v0));
                ...
                if(count($glob) < 100){
                    ...
                    foreach($glob as $k1 => $v1){
                        ...
                        $tabl[$nom_court_de_l_element]=array( 'sur_disque' => true, 'en_base' => 0, 'type_fichier' => false, 'nom_relatif_dossier' => '');
            ...
            if(($_SESSION[APP_KEY]['cible_courante']['chp_nom_cible'] === 'fta') && ($_SESSION[APP_KEY]['cible_courante']['chp_dossier_cible'] !== 'fta')){
                ...
                if(is_dir($chemin_relatif)){
                    ...
                    /* echo __FILE__ . ' ' . __LINE__ . ' __LINE__ = <pre>' . var_export( $chemin_reel , true ) . '</pre>' ; exit(0);*/
                    $glob=array();
                    /* la fonction glob ne prend pas en compte le fichier .htaccess */
                    $chemin_du_repertoire=$chemin_reel.'/';
                    $monscan=scandir($chemin_du_repertoire);
                        ...
                        if(count($glob) < 100){
                            ...
                            foreach($glob as $k1 => $v1){
                                ...
                                if(isset($tabl[$nom_fichier])){
                                    ...
                                    if(($extension === '.php') || ($extension === '.js') || ($extension === '.html') || ($extension === '.htm') || ($extension === '.htm') || ($extension === '.json') || ($extension === '.bat') || ($extension === '.rev') || ($extension === '.css') || ($extension === '.sql') || ($extension === '.txt') || ($extension === '.db') || ($extension === '.htaccess')){
